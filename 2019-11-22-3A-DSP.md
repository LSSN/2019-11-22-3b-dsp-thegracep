...
```
Il teorema del campionamento  afferma che per campionare correttamente un segnale a banda limitata, è necessario campionarlo con una frequenza di campionamento pari almeno al doppio della massima frequenza del segnale. La sua formula è f>fmax.
```
...
// numero di entrate 4
// numero di uscite 8
 *(0.5),*(0.25),*(0.125),*(0.1) <: _,_,_,_,_,_,_,_;
...
freq = 110;
process = os.osc(freq);
os.osc(freq*1), os.osc(freq*3), os.osc(freq*5), os.osc(freq*7), os.osc(freq*9), os.osc(freq*11), os.osc(freq*13), os.osc(freq*15), os.osc(freq*17) <:(9);
```
